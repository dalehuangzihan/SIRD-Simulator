...
class ExperimentOutputRaw:
    ...
    def _union_length(self, intervals):
        """
        intervals are list of (start_s, end_s) tuples.
        Return total length (s) of tuples' union.
        """
        if not intervals:
            return 0.0
    ...
    def process_results_fct(self):
        ...
        applied_intervals = []   # is [(start_s, last_send_s)]
        achieved_intervals = []  # is [(start_s, last_recv_s)]
        ...
        total_bytes_rcvd_B = total_rcvd_B
...
class PoissonIntervalGenerator:
    ...
    def solve_lambda_for_mean_discrete(self, mu, L, U):
        """solve for lambda so that the mean of discrete truncated exponential = mu."""
        if not (L <= mu <= U):
        ...
        # "bracket-in" lambda value
        low, high = -10.0, 10.0  # allow negative lambda if mu > mid
    ...
    def discrete_trunc_exp_pmf(self, x, lam, L, U):
        """get PMF value for integer x in [L,U]"""
        vals = np.arange(L, U+1)
        probs = np.exp(-lam * (vals - L))
        probs /= probs.sum()
    ...
    def make_feasible_initial_int(self, n, L, U, S):
        """construct integer vector with sum = S"""
        if not (n * L <= S <= n * U):
    ...
    def mcmc_trunc_exp_cond_mean_discrete(self, lam, L, U, n, mu_target):
        """is MCMC sampler for integer truncated exponential conditional on exact mean"""
        ...
        for step in range(self.num_mcmc_iters):
            ...
            s_pair = x[i] + x[j]
            # get feasible int range for x[i]
...
class EmpiricalDistr(ABC):
    ...
    def load_cdf_from_file(filename):
        ...
        with open(filename, 'r') as f:
            ...
            try:
                # first line of file could be a mean or scaling value; if so, ignore, continue reading actual CDF entries
                float(first_line)  
            except ValueError:
                # first line is part of actual CDF
