...
def experiment_vary_byteloadsize_largebload_multiflow(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    ...
    KILOBYTE = 1000
    # byteload_size_KB_list = [400]
    byteload_size_KB_list = [4, 40, 400] # 4KB to 400KB byteloads
    ...
    ssird_sim_dur_list = [0.012] * num_of_experiments # TODO: tweak
    dctcp_sim_dur_list = [0.012] * num_of_experiments # TODO: tweak
...
def experiment_vary_byteloadsize_largebload_multiflow_extended_31flo(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    experiment_family = f"FCT_Large_Byteloads{title_addendum}"
    proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME, dale_experiment_rig.DCTCP_PROTO_NAME]
    # proto_names = [dale_fct_experiment.SSIRD_PROTO_NAME]
    # proto_names = [dale_experiment_rig.DCTCP_PROTO_NAME]
    ...
    minimum_interval_us = 1 # 1us
    total_flow_size_B = 2000000 # 2MB
    # NOTE: smallest byteload size that SSIRD sim is set up to use is 4B => with headers this makes a total of 64B per byteload.
    # NOTE: byteload sizes are multiples of 4 so that the inter-byteload periods are later calculated to be whole numbers in the end

    KILOBYTE = 1000
    byteload_size_KB_list = [0.2, 2, 20, 200, 2000] # 4KB to 400KB byteloads
    byteload_size_B_list = [int(n * KILOBYTE) for n in byteload_size_KB_list] 
    num_of_experiments = len(byteload_size_B_list)
    # num_flows = 1
    num_flows = 31 # TODO: for testing
    inter_flow_spacing_us = 0 # TODO: for testing
    flow_start_times_us_list = [i * inter_flow_spacing_us for i in range(0, num_flows)]
    ...
    ssird_sim_dur_list = [0.012, 0.012, 0.012, 0.012, 0.01]
    dctcp_sim_dur_list = [0.022, 0.012, 0.012, 0.012, 0.01]
...
def experiment_vary_byteloadsize_largebload_multiflow_150flo(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    experiment_family = f"FCT_Large_Byteloads{title_addendum}"
    proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME, dale_experiment_rig.DCTCP_PROTO_NAME]
    # proto_names = [dale_fct_experiment.SSIRD_PROTO_NAME]
    # proto_names = [dale_experiment_rig.DCTCP_PROTO_NAME]
    ...
    # minimum_interval_us = 1
    minimum_interval_us = 100 # 100us
    # total_flow_size_B = 4000000 # 4MB
    total_flow_size_B = 400000 # 0.4MB
    # NOTE: smallest byteload size that SSIRD sim is set up to use is 4B => with headers this makes a total of 64B per byteload.
    # NOTE: byteload sizes are multiples of 4 so that the inter-byteload periods are later calculated to be whole numbers in the end
    ...
    # byteload_size_KB_list = [400]
    # byteload_size_KB_list = [4, 40, 400] # 4KB to 400KB byteloads
    byteload_size_KB_list = [4, 40] # 4KB to 40KB byteloads
    byteload_size_B_list = [int(n * KILOBYTE) for n in byteload_size_KB_list] 
    num_of_experiments = len(byteload_size_B_list)
    # num_flows = 1
    ...
    ssird_sim_dur_list = [0.03] * num_of_experiments # TODO: tweak
    dctcp_sim_dur_list = [0.03] * num_of_experiments # TODO: tweak
...
def experiment_vary_byteloadsize_subpkt_multiflow_150flo(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    experiment_family = f"FCT_Subpkt_Byteloads{title_addendum}"
    proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME, dale_experiment_rig.DCTCP_PROTO_NAME]
    # proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME]
    # proto_names = [dale_experiment_rig.DCTCP_PROTO_NAME]
    ...
    # logger.debug(f"Sim Durations list (SSIRD): {ssird_sim_dur_list}")
    # assert(len(ssird_sim_dur_list) == num_of_experiments)
    ssird_sim_dur_list = [0.015] * num_of_experiments
    
    # dctcp_sim_dur_multiplication_factor = 200
    # dctcp_sim_dur_list = []
    # for i in range(0, len(num_byteloads_per_flow_list)):
    #    dctcp_sim_dur_list.append(dale_experiment_rig.Experiment.get_sim_duration(num_flows, inter_flow_spacing_us, num_byteloads_per_flow_list[i], byteload_size_B_list[i], inter_byteload_period_us_list[i], dctcp_sim_dur_multiplication_factor))
    dctcp_sim_dur_list = [0.06] * num_of_experiments # TODO: tweak
    logger.debug(f"Sim Durations list (DCTCP): {dctcp_sim_dur_list}")
    assert(len(dctcp_sim_dur_list) == num_of_experiments)
...
def experiment_vary_byteloadsize_fullrange_multiflow(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    experiment_family = f"FCT_Subpkt_Byteloads{title_addendum}"
    proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME, dale_experiment_rig.DCTCP_PROTO_NAME]
    # proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME]
    # proto_names = [dale_experiment_rig.DCTCP_PROTO_NAME]
    ...
    # NOTE: byteload sizes are multiples of 4 so that the inter-byteload periods are later calculated to be whole numbers in the end
    byteload_size_B_list = [4]
    # byteload_size_B_list = [4, 40, 400, 4000, 40000, 400000] # 4B to 400KB 
    num_of_experiments = len(byteload_size_B_list)
    num_flows = 15 # TODO: for testing
    inter_flow_spacing_us = 0 # TODO: for testing
    flow_start_times_us_list = [i * inter_flow_spacing_us for i in range(0, num_flows)]
...
def experiment_vary_byteloadsize_large_bload_20MBflo_5flo_8gbps_total(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    ...
    minimum_interval_us = 10 # 10us
    total_flow_size_B = 20000000 # 20MB
    # NOTE: smallest byteload size that SSIRD sim is set up to use is 4B => with headers this makes a total of 64B per byteload.
    # NOTE: byteload sizes are multiples of 4 so that the inter-byteload periods are later calculated to be whole numbers in the end

    KILOBYTE = 1000
    byteload_size_KB_list = [2, 20, 200, 2000, 20000] # 2KB to 20MB byteloads
    byteload_size_B_list = [int(n * KILOBYTE) for n in byteload_size_KB_list] 
    num_of_experiments = len(byteload_size_B_list)
    num_flows = 5 # for 8Gbps total app goodput
    inter_flow_spacing_us = 0 # TODO: for testing
    flow_start_times_us_list = [i * inter_flow_spacing_us for i in range(0, num_flows)]
    ...
    assert num_of_experiments == len(ssird_fct_list)
    assert num_of_experiments == len(dctcp_fct_list)
def experiment_vary_byteloadsize_fullrange_multiflow_1flo_1pt6gbps_total(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    ...
    KILOBYTE = 1000
    # byteload_size_KB_list = [200] # 20B to 200KB byteloads
    byteload_size_KB_list = [0.02, 0.2, 2, 20, 200] # 20B to 200KB byteloads
    byteload_size_B_list = [int(n * KILOBYTE) for n in byteload_size_KB_list] 
    num_of_experiments = len(byteload_size_B_list)
    num_flows = 1 # for 1.6Gbps total app goodput
    inter_flow_spacing_us = 0 # TODO: for testing
    flow_start_times_us_list = [i * inter_flow_spacing_us for i in range(0, num_flows)]
    ...
    dctcp_sim_dur_list = [0.003, 0.003, 0.003, 0.003, 0.003]
...
def experiment_vary_byteloadsize_fullrange_multiflow_3flo_4pt8gbps_total(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    experiment_family = f"FCT_Fullrange_Byteloads{title_addendum}"
    proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME, dale_experiment_rig.DCTCP_PROTO_NAME]
    # proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME]
    # proto_names = [dale_experiment_rig.DCTCP_PROTO_NAME]
    ...
    minimum_interval_us = 0.1 # 0.1us
    # total_flow_size_B = 20000 # 20KB # FOR TESTING ONLY
    total_flow_size_B = 200000 # 200KB
    # NOTE: smallest byteload size that SSIRD sim is set up to use is 4B => with headers this makes a total of 64B per byteload.
    # NOTE: byteload sizes are multiples of 4 so that the inter-byteload periods are later calculated to be whole numbers in the end

    KILOBYTE = 1000
    # byteload_size_KB_list = [0.02]
    byteload_size_KB_list = [0.02, 0.2, 2, 20, 200] # 20B to 200KB byteloads
    byteload_size_B_list = [int(n * KILOBYTE) for n in byteload_size_KB_list] 
    num_of_experiments = len(byteload_size_B_list)
    num_flows = 3 # for 4.8Gbps total app goodput
    inter_flow_spacing_us = 0 # TODO: for testing
    flow_start_times_us_list = [i * inter_flow_spacing_us for i in range(0, num_flows)]
    ...
    # ssird_sim_dur_list = [0.001] # for 5us RTT
...
def experiment_vary_byteloadsize_fullrange_multiflow_5flo_8gbps_total(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    experiment_family = f"FCT_Fullrange_Byteloads{title_addendum}"
    proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME, dale_experiment_rig.DCTCP_PROTO_NAME]
    # proto_names = [dale_experiment_rig.SSIRD_PROTO_NAME]
    # proto_names = [dale_experiment_rig.DCTCP_PROTO_NAME]
...
def experiment_vary_byteloadsize_fullrange_multiflow_31flo(is_full_postproc=True, title_addendum="", log_level=dale_experiment_rig.LOG_LEVEL_2):
    ...
    KILOBYTE = 1000
    byteload_size_KB_list = [200] # TODO: for testing
    # byteload_size_KB_list = [0.02, 0.2, 2, 20, 200] # 20B to 200KB byteloads
    byteload_size_B_list = [int(n * KILOBYTE) for n in byteload_size_KB_list] 
    num_of_experiments = len(byteload_size_B_list)
    num_flows = 31 # for 49.6Gbps total app goodput
    inter_flow_spacing_us = 0 # TODO: for testing
    flow_start_times_us_list = [i * inter_flow_spacing_us for i in range(0, num_flows)]
    ...
    ssird_sim_dur_list = [0.0]
    dctcp_sim_dur_list = [0.01] # TODO: for testing
...
if __name__ == "__main__":
    ...
    ''' 5us RTT'''
    experiment_vary_byteloadsize_fullrange_multiflow_31flo(is_full_postproc=False, title_addendum="_fullrange_31flo_credit_steal", log_level=dale_experiment_rig.LOG_LEVEL_6)
