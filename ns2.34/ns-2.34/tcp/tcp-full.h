...
{
public:
	FullTcpAgent() : prio_scheme_(0), prio_num_(0), startseq_(0), last_prio_(0), seq_bound_(0),
...
protected:
	...
	*/
	...
	virtual void oldack()
	{ // what to do on old ack
		dupacks_ = 0;
	}

	virtual void extra_ack()
	{ // dup ACKs after threshold
		if (reno_fastrecov_)
			cwnd_++;
	}
...
{
public:
	SackFullTcpAgent() : sq_(sack_min_), sack_min_(-1), h_seqno_(-1) {}
