...
void XPassAgent::recv_data(Packet *pkt)
{
    ...
    /* Dale: set is_xpass_app_msg_ */
    req_id.is_xpass_app_msg_ = r2p2_hdr->is_xpass_app_msg();
...
Packet *XPassAgent::construct_data(Packet *credit)
{
    ...
    if (cur_req_id_tup_ != nullptr)
    {
        ...
        slog::log6(debug_, addr(), "flow_id=", cur_req_id_tup_->flow_id_, "src=", cur_req_id_tup_->cl_addr_, "dst=", cur_req_id_tup_->sr_addr_, "is_final_req_of_conn_=", cur_req_id_tup_->is_final_req_of_conn_, "curr_app_data_sent_=", curr_app_data_sent_, "total_msg_data_=", cur_req_id_tup_->total_msg_data_);
		/* Dale: try: set is_final_req_of_conn flag in header; curr_app_data_sent_ >= total_msg_data_ can happen sometimes...? */
		r2p2_hdr->is_final_req_of_conn() = (cur_req_id_tup_->is_final_req_of_conn_ && curr_app_data_sent_ >= cur_req_id_tup_->total_msg_data_);
        /* Dale: track if this is an xpass app message */
        r2p2_hdr->is_xpass_app_msg() = true;
